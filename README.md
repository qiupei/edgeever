...
- 项目形态：Bun workspace monorepo，包含 `apps/web`、`apps/api` 和 `packages/shared`。
- 前端应用：Vite、React、React Router、TanStack Query、TypeScript。
- UI 与交互：Tailwind CSS、shadcn/ui 风格组件、Radix UI、Vaul、cmdk、lucide-react。
- 富文本与内容：TipTap / ProseMirror、Markdown / TipTap JSON 双向转换。
- PWA 与离线能力：vite-plugin-pwa、Workbox、Dexie，本地草稿与同步队列存放在 IndexedDB。
- 图片与资源管理：浏览器端图片压缩、react-dropzone、yet-another-react-lightbox；资源文件存储在 Cloudflare R2。
- 后端 API：Cloudflare Workers、Hono、hono/zod-validator、Zod、REST API、OpenAPI schema、Remote MCP / Streamable HTTP endpoint。
- 数据与云资源：Cloudflare D1、Cloudflare R2、Cloudflare Workers Assets、Cloudflare Workers AI binding。
- 导入与脚本工具：Evernote ENEX 导入脚本、fast-xml-parser、Turndown、sharp、stdio MCP bridge。
- 工具链：Bun、Wrangler、TypeScript、Vite、Tailwind CSS。
